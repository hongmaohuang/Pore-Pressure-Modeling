...
ATM_TXT_PATH = "/Users/hmhuang/Sth_should_be_local/Iceland/datasets/atm_grvik_1361.csv"
ERA5_SNOW_CSV_PATH = "/Users/hmhuang/Sth_should_be_local/Iceland/datasets/era5/reanalysis-era5-land-timeseries-sfc-snow6__1_fke.csv"
...
# ERA5-Land time-series "snow depth" is snow thickness, not SWE.
# This density converts snow depth to approximate surface load.
...
def read_imo_monthly_pressure_txt(filepath):
    ...
    if str(filepath).lower().endswith(".csv"):
        ...
        pressure_column = "ps" if "ps" in df.columns else "p"
        if pressure_column not in df.columns:
            raise ValueError(f"No pressure column found in {filepath}")
        df[pressure_column] = pd.to_numeric(df[pressure_column], errors="coerce")
        df = df.dropna(subset=["time", pressure_column]).set_index("time").sort_index()
        df["patm_pa"] = df[pressure_column] * 100.0
        return df[["patm_pa"]]
...
def read_era5_snow_csv(filepath):
    """
    Read ERA5-Land time-series snow depth and snow-cover data.
    """
    df = pd.read_csv(filepath, parse_dates=["valid_time"])
    required_columns = ["valid_time", "sde"]
    missing = [column for column in required_columns if column not in df.columns]
    if missing:
        raise ValueError(f"Missing ERA5 snow columns in {filepath}: {missing}")

    df["sde"] = pd.to_numeric(df["sde"], errors="coerce")
    if "snowc" in df.columns:
        df["snowc"] = pd.to_numeric(df["snowc"], errors="coerce")

    if df["sde"].notna().sum() == 0:
        latitude = df["latitude"].iloc[0] if "latitude" in df.columns and len(df) else None
        longitude = df["longitude"].iloc[0] if "longitude" in df.columns and len(df) else None
        raise ValueError(
            "ERA5 snow-depth column contains no usable values. "
            f"File: {filepath}. "
            f"Downloaded grid point: latitude={latitude}, longitude={longitude}. "
            "This usually means the ERA5-Land time-series point is over ocean. "
            "Download a nearby land grid point before fitting the snow-loading model."
        )
    ...
    out = (
        df.rename(
            columns={
                "valid_time": "datetime",
                "sde": "era5_snow_depth_m",
                "snowc": "era5_snow_cover",
            }
        )
        .set_index("datetime")
        .sort_index()
    )
    columns = ["era5_snow_depth_m"]
    if "era5_snow_cover" in out.columns:
        columns.append("era5_snow_cover")
    return out[columns]
...
def run_pore_pressure_workflow(
    ...
    atm_txt_path,
    output_csv_path,
    era5_snow_csv_path=ERA5_SNOW_CSV_PATH,
    ...
    if include_snow_loading:
        era5_snow = read_era5_snow_csv(era5_snow_csv_path)
        snow_rs = prepare_time_series(
            era5_snow[["era5_snow_depth_m"]],
            start=start,
            end=end,
            rule=resample_rule,
            target_index=model_index,
            dataset_name="ERA5 snow depth",
            filepath=era5_snow_csv_path,
        )
        if "era5_snow_cover" in era5_snow.columns:
            snow_cover_rs = prepare_time_series(
                era5_snow[["era5_snow_cover"]],
                start=start,
                end=end,
                rule=resample_rule,
                target_index=model_index,
                dataset_name="ERA5 snow cover",
                filepath=era5_snow_csv_path,
            )
    ...
    if snow_rs is not None:
        out["era5_snow_depth_m"] = snow_rs.values
        out["era5_snow_density_kg_m3"] = snow_density_kg_m3
    if snow_cover_rs is not None:
        out["era5_snow_cover"] = snow_cover_rs.values
