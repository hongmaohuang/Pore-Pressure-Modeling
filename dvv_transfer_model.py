...
def build_predictor_dataframe(
    ...
    if include_snow_loading:
        required_columns.append(f"Pp_snow_z{depth_label}m_pa")
    ...
    if include_snow_loading:
        predictors["pp_snow_pa"] = pore_df[f"Pp_snow_z{depth_label}m_pa"]
...
def run_dvv_transfer_workflow(
    ...
    hydraulic_columns = [
        col for col in ["pp_gwl_pa", "pp_atm_pa", "pp_snow_pa"] if col in merged.columns
    ]
    merged["pp_total_pa"] = merged[hydraulic_columns].sum(axis=1)
