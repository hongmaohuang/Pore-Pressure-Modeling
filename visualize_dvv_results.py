...
def make_observed_modeled_plot(df, output_path, y_lim):
    ...
    ax_snow.spines["right"].set_position(("axes", 1.40))
    ...
    if "pp_snow_pa" in df.columns:
        ax_snow.plot(
            ...
            linewidth=0.9,
            linestyle=(0, (3, 1, 1, 1)),
            label="Snow pore pressure",
        )
        ax_snow.set_ylabel("Snow pore pressure (Pa)", color="tab:cyan")
        ax_snow.tick_params(axis="y", colors="tab:cyan")
    ...
    handles6, labels6 = ax_snow.get_legend_handles_labels()
    ax.legend(
        handles1 + handles2 + handles3 + handles4 + handles5 + handles6,
...
def make_predictor_plot(df, output_path):
    predictor_columns = [
        col
        for col in ["pp_gwl_pa", "pp_atm_pa", "pp_snow_pa", "dvv_temp_thermoelastic"]
